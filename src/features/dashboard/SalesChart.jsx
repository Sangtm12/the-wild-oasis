...
function SalesChart({ bookings, numDays }) {
  ...
  for (let i = 1; i <= numDays; i++) {
    const dayObj = {};
    const currentCheckingDate = subDays(new Date(), numDays - i);

    dayObj.label = format(currentCheckingDate, "MMM dd");
    dayObj.totalSales = bookings.reduce(
      (acc, cur) =>
        isSameDay(cur.created_at, currentCheckingDate)
          ? acc + cur.totalPrice
          : acc,
      0
    );
    dayObj.extrasSales = bookings.reduce(
      (acc, cur) =>
        isSameDay(cur.created_at, currentCheckingDate)
          ? acc + cur.extrasPrice
          : acc,
